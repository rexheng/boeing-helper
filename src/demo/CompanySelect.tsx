...
export function CompanySelect({ onSelect }: Props) {
  ...
  const lookupActive = lookupQuery.trim().length >= 2
    ...
    <div>
      <div className="text-center max-w-2xl mx-auto mb-7">
        ...
        <p className="mt-3 text-base" style={{ color: "var(--text-secondary)" }}>
          Choose a region and country, or search any Boeing partner by name.
        </p>
      </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-xs tabular-nums" style={{ color: "var(--text-muted)" }}>
                        {hasOrgs ? `${orgs.length} partner${orgs.length === 1 ? "" : "s"}` : "Search above"}
                      </span>
                      <ChevronDown
                        size={16}
                        style={{
                  ...
                  {open && (
                    <div className="px-3 pb-3 pt-1">
                      {hasOrgs ? (
                        ...
                        <p className="px-1 py-2 text-sm" style={{ color: "var(--text-secondary)" }}>
                          No listed partners for {country.name}. Use search above for ministries and airlines.
                        </p>
                      )}
                    </div>
                  )}
      ...
      {lookupActive && selected && (
        <p className="max-w-4xl mx-auto mt-4 text-sm font-medium" style={{ color: "var(--boeing-navy)" }}>
          Selected — continue with Next, or clear search to browse by region.
        </p>
      )}
    </div>
  )
